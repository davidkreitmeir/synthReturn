...
```math
  ...
  \text{s.t.} \quad  \underset{j \in \text{Control group}}{ \sum w_{j}^{i} = 1} \text{and}
   w_{j}^{i} \geq 0
```
...
```math
    ...
    \text{s.t.} \quad
    \mathbf{A}_{1}\mathbf{w} = 1 \text{and}
...
```math
    \widehat{\phi}\left(0,k\right) = \frac{\underset{i \in \text{Treatment group}}{\sum \frac{\sum_{t=0}^{\tau_{2}} \widehat{AR}_{it}}{\widehat{\sigma_{i}}}}}{\underset{i \in \text{Treatment group}}{\sum \frac{1}{\widehat{\sigma_{i}}}}} \quad
